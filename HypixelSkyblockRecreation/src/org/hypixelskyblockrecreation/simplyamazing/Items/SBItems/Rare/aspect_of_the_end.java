...
public class aspect_of_the_end extends SkyBlockItem {

	public aspect_of_the_end(Material material, String name, String description, Rarity rarity, Type type, boolean stackable, boolean oneTimeUse, List<SBAbility> abilities, CustomRecipe craftingRecipe) {
	...
	public boolean rightClickAction(Player p0, ItemStack p1) {
        ...
        l.add(p.getEyeLocation().getDirection().multiply(8));
        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {
        	l = p.getLocation().clone();
	        l.add(p.getEyeLocation().getDirection().multiply(7));
	        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {
	        	l = p.getLocation().clone();
		        l.add(p.getEyeLocation().getDirection().multiply(6));
		        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {
		        	l = p.getLocation().clone();
			        l.add(p.getEyeLocation().getDirection().multiply(5));
			        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {
			        	l = p.getLocation().clone();
				        l.add(p.getEyeLocation().getDirection().multiply(4));
				        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {
				        	l = p.getLocation().clone();
					        l.add(p.getEyeLocation().getDirection().multiply(3));
					        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {
					        	l = p.getLocation().clone();
						        l.add(p.getEyeLocation().getDirection().multiply(2));
						        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {
						        	l = p.getLocation().clone();
							        l.add(p.getEyeLocation().getDirection().multiply(1));
							        if(!(l.getBlock().isEmpty() || l.getBlock().isLiquid())) {}
						        }
					        }
				        }
